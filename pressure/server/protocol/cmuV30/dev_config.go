...
type DevConfigWrapper struct {
	...
	PowOutChFun      map[string]int   `json:"PowOutChFun"`
	Threshold        Threshold        `json:"Threshold"`
	LightRules       map[string]int   `json:"LightRules"`
	StrobeRules      map[string]int   `json:"StrobeRules"`
	Net              Net              `json:"Net"`
	...
	TransferMode     int              `json:"TransferMode"`
}
...
type Threshold struct {
	...
	PowModCurs    []PowModCur `json:"PowModCur"`
	SwitchMinSpds []SwitchSpd `json:"SwitchMinSpd"`
}

type PowModCur struct {
	Ch     int `json:"Ch"`
	MinCur int `json:"MinCur"`
}

type SwitchSpd struct {
	Ch     int `json:"Ch"`
...
func NewDevConfig(number string) *DevConfigWrapper {
	return &DevConfigWrapper{
		...
		Threshold: Threshold{
			...
			Roll:          11,
			Pitch:         11,
			PowModCurs:    []PowModCur{},
			SwitchMinSpds: []SwitchSpd{},
		},
		LightRules:  emptyIntMap,
		StrobeRules: emptyIntMap,
		Net: Net{
			Local: Local{
				IP:     "127.0.0.1",
